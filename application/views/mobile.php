...
else if ($type == "menu" && $room == "all" && $time_group == "all")
{
	...
	echo "<hr/>";
	echo "<h2>Full Program</h2>";
	echo anchor("start/search/year","View and search",Array("class" => "btn btn-primary btn-lg mb-3"));
	echo "<hr/>";
	echo "<br/>";
	echo "<h2>Talks</h2>";
	...
	foreach($tg as $time_group)
		{
			...
			foreach(explode(",",$rooms) as $room)
			{
				if (!$this->Entry->room_is_empty($year,$room,$time_group))
				{
					...
					$affiliation_summary = $this->Entry->get_room_affiliation_summary_short($year,$room,$time_group);
